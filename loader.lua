--[[
    ...
    Key: DIKZPROJECT
    Fitur: Key system, GUI, Bypass anti-cheat, Floating logo (Min Amin & cina bukitzz)
    All-in-one – langsung jalan
]]
...
-- Bypass Anti-Cheat (biar tidak kena ban/kick)
...
-- ========== 2. FLOATING LOGO (dari foto) ==========
local function CreateFloatingLogo()
    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "FloatingLogo_MinAmin"
...
-- ========== 3. MAIN SCRIPT (REDZ HUB GUI) ==========
local function LoadMainScript()
    keyValid = true
    StarterGui:SetCore("SendNotification", {Title = "✅ REDZ HUB", Text = "KEY BENAR, SELAMAT BERMAIN.....", Duration = 3})
    BypassAntiCheat()
    CreateFloatingLogo() -- muncul logo mengambang
    
    -- Load library UI (dari raw github)
    ...
    AutoFarm:Toggle("Auto Farm Mastery", nil, function(state)
        while state and keyValid do
            task.wait(0.5)
            local args = {[1] = "Farm", [2] = "Mastery"}
            pcall(function()
                game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Main"):FireServer(unpack(args))
            end)
        end
    end)
    
    Misc:Button("Bypass Anti-Cheat (Manual)", function()
        BypassAntiCheat()
    ...
    StarterGui:SetCore("SendNotification", {Title = "REDZ HUB", Text = "Script berjalan! Nikmati fitur.", Duration = 2})
end

-- ========== 4. TAMPILAN KEY SYSTEM GUI ==========
...
CheckButton.MouseButton1Click:Connect(function()
    ...
    if inputKey == correctKey then
        StatusLabel.Text = "✅ KEY BENAR, SELAMAT BERMAIN....."
